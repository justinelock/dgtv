...
_ss_sh_link="https://raw.githubusercontent.com/justinelock/dgtv/master/ss_install.sh"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "请输入$yellow Shadowsocks $none密码...(默认密码: ${cyan}fuckccp.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="fuckccp.com"
		case $sspass in
		*[/$]*)
			echo
...
install() {
	if [[ -f $backup ]]; then
		echo
		echo -e " 已经安装...输入$cyan ss $none即可管理"
		echo
		exit 1
	fi
	shadowsocks_port_config
